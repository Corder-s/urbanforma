...
interface Props {
  ...
  currentName: string;
  /** Module label under the project name (defaults to the visualization module). */
  subtitle?: string;
  onSwitch: (id: string) => void;
}

/** Toolbar project selector (keyboard-accessible menu), mirrors the studio switcher. */
export function VisualizationProjectSwitcher({ projects, currentId, currentName, subtitle = "GIS & 3D Visualization", onSwitch }: Props) {
    ...
    <div ref={ref} className="relative min-w-0">
      ...
      >
        <span className="min-w-0">
          ...
          <span className="block truncate text-[11px] font-semibold leading-tight text-primary">{subtitle}</span>
        </span>
