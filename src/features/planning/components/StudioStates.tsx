...
import { ArrowLeft, FolderOpen, Map, RefreshCw, SearchX, Shapes, TriangleAlert } from "lucide-react";
import { Button } from "../../../components/ui/Button";
import type { PlanningProjectSummary } from "../services/planning.service";

/** Studio-wide loading state (replaces the whole workspace). */
export function StudioLoading({ label = "Loading project…" }: { label?: string }) {
  return (
    <div role="status" aria-live="polite" className="grid h-full min-h-[320px] place-items-center bg-canvas p-6">
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-muted">Preparing the planning canvas</p>
        </div>
      </div>
    </div>
  );
...
/** Canvas-only skeleton shown while a document loads inside the workspace frame. */
export function CanvasLoading() {
  return (
    <div role="status" aria-live="polite" className="relative h-full w-full overflow-hidden bg-[#F1F5FB]">
      <div
      ...
      <div className="absolute inset-0 grid place-items-center p-6">
        <div className="flex items-center gap-3 rounded-2xl border border-line bg-surface/95 px-4 py-3 shadow-soft">
          <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-primary motion-reduce:animate-none" aria-hidden="true" />
          <span className="text-sm font-semibold text-ink">Canvas loading…</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="grid h-full min-h-[360px] place-items-center overflow-y-auto bg-canvas p-6">
      <div className="w-full max-w-lg rounded-3xl border border-line bg-surface p-6 text-center shadow-card sm:p-8">
        ...
        <p className="mt-2 text-[15px] leading-relaxed text-muted">
          Planning Studio works on one project at a time. Open a project from the Projects page or pick one to start.
        </p>
        {projects.length > 0 && (
  ...
  return (
    <div role="alert" className="grid h-full min-h-[360px] place-items-center bg-canvas p-6">
      <div className="w-full max-w-md rounded-3xl border border-line bg-surface p-6 text-center shadow-card sm:p-8">
        ...
        <h2 className="mt-5 text-xl font-extrabold text-ink">Couldn't load the plan</h2>
...
/** Shown over the canvas when a document has no objects yet. */
  ...
  return (
    <div className="pointer-events-none absolute inset-0 grid place-items-center p-6">
      <div className="pointer-events-auto max-w-sm rounded-2xl border border-line bg-surface/95 p-5 text-center shadow-soft">
        ...
        <h3 className="mt-3 text-[15px] font-extrabold text-ink">No planning objects yet</h3>
        <p className="mt-1 text-[13px] leading-relaxed text-muted">
          Pick a tool on the left — for example <strong className="font-bold text-ink">Building</strong> — and click inside the site boundary to place your first element.
        </p>
      </div>
    </div>
  );
